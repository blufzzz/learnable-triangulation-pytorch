...
length = 500
subjects = ['S9']#['subject_names']
actions = ['Directions-1'] #retval['action_names']
MPJPE = None
...
for subject in subjects:
    for action in actions:
        ...
        subject_action_path = os.path.join(experiment_root,'results', f'{subject}_{action}') 
        ...
        stop=stop_frame_indxs[offset:][action_index].item()
        ...
        if MPJPE is not None:
            ax1.plot(MPJPE, '--', color='black', label='Overall MPJPE')
        ...
        # DIFFS #   
        #########
        ax2.plot(model_diffs, label='Model diffs', linewidth=6)
        ...
        ax2.legend()
        plt.savefig(os.path.join(subject_action_path, f'{subject}_{action}_err_diffs'), dpi=280)
        ...
        if RETURN_STYLE_VECTOR:
            ...
            plt.savefig(os.path.join(subject_action_path, f'{subject}_{action}_hist'), dpi=280)
            plt.close('all')
            ...
            if experiment_type[-2:] == '2d':
                ...
                plt.savefig(os.path.join(subject_action_path, f'{subject}_{action}_style_feature_maps_frame={n_frame}'), dpi=280)        
                plt.close('all')
                ...
                plt.savefig(os.path.join(subject_action_path, f'{subject}_{action}_style_volume_slice_t={time_step}_frame={n_frame}'), dpi=280)       
                plt.close('all')
            ...
            plt.savefig(os.path.join(subject_action_path, f'{subject}_{action}_ptp'), dpi=280)    
            plt.close('all')
            ...
            plt.xlabel('feature index')
            plt.savefig(os.path.join(subject_action_path, f'{subject}_{action}_var'), dpi=280)    
            plt.close('all')
            ...
            plt.savefig(os.path.join(subject_action_path, f'{subject}_{action}_style_diffs'), dpi=280)     
            plt.close('all')
        ...
        #########
        if MAKE_VIDEOS:    
            keypoints_dir = os.path.join(subject_action_path,'images_videos_lowres')
