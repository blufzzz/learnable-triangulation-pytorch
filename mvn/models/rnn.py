...
class KeypointsRNNCell(nn.Module):
    def __init__(self, 
                in_planes, 
                out_planes, 
                hidden_dim=256, 
        ...
        super(KeypointsRNNCell, self).__init__()
        ...
        if layer_norm:
            ...
            self.rnn = script_lnlstm(in_planes, 
                                     hidden_dim, 
                                     num_layers, 
                                     bias=True,
                                     batch_first=self.batch_first,
                                     dropout=droupout_rate if use_dropout else False, 
                                     bidirectional=bidirectional,
                                     decompose_layernorm=False)

        else:
            self.rnn = nn.LSTM(in_planes, 
                                hidden_dim, 
                                batch_first=self.batch_first, 
                                num_layers=num_layers, 
                                bidirectional=bidirectional,
                                dropout=droupout_rate if use_dropout else 0)
    ...
    def forward(self, keypoints, hidden): # mean=None, std=None
        ...
        if not self.batch_first:
            out = out.view(dt, bs, self.out_planes)
