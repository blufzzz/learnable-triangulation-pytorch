...
class VolumetricTemporalNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.use_v2v_3 = config.use_v2v_3 if hasattr(config, "use_v2v_3") else False
        ...
        self.timestep_multiplier = config.timestep_multiplier if hasattr(config, "timestep_multiplier") else 1.0
        ...
        self.transfer_cmu_to_kth = config.transfer_cmu_to_kth if hasattr(config, "transfer_cmu_to_kth") else False

        # modules
        self.backbone = pose_resnet.get_pose_net_2(config.backbone)
        ...
        self.v2v_input_channels = 32 #32*self.dt if config.volume_aggregation_method == 'no_aggregation' else 32
        self.volume_net = V2VModel_3(self.v2v_input_channels, self.num_joints) if self.use_v2v_3 else V2VModel_2(self.v2v_input_channels, self.num_joints)
    ...
    def get_coord_volumes(self, 
        ...
        base_point = torch.zeros((*bs_dt, 3), dtype=torch.float).to(device)
    ...
    def forward(self, images_batch, batch, root_keypoints=None):
        ...
        if self.return_heatmaps:
            heatmaps = heatmaps.view(batch_size, dt, *heatmaps.shape[1:])
        ...
        if self.use_precalculated_pelvis:
            ...
            coord_volumes, _ = self.get_coord_volumes(self.cuboid_side*self.cuboid_multiplier,
                                                      self.volume_size, 
                                                      device, 
        ...
        coord_volumes, cuboids = self.get_coord_volumes(self.cuboid_side,
                                                        self.volume_size, 
                                                        device,
        ...
        #     volumes_keypoints_3d = []
        #     volumes_final = []
        #     for i in range(dt):
        #         volume_final = self.volume_net(volumes[:,i,...])
        #         volume_keypoints_3d, volume_final = op.integrate_tensor_3d_with_coordinates(volume_final * self.volume_multiplier, coord_volumes, softmax=self.volume_softmax)

        #         volumes_final.append(volume_final)
        #         volumes_keypoints_3d.append(volume_keypoints_3d)
        ...
        # inference
        volumes_final = self.volume_net(volumes)
        # integral 3d
        vol_keypoints_3d, volumes_final = op.integrate_tensor_3d_with_coordinates(volumes_final * self.volume_multiplier, coord_volumes, softmax=self.volume_softmax)
